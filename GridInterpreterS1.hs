...
type ColorTup = (((Int,Int,Int),(Int,Int,Int)),Mem)
type RColorTup = ((Int,Int,Int),(Int,Int,Int))
type Grid = Vc.Vector (Vc.Vector Cell)

data Cell = Cell { cellMem :: ColorTup,
                   effectiveColor :: ColorTup,
                   isWritable :: Bool,
                   isPassable :: Bool } deriving (Eq)
...
instance FromJSON RoboConfig where
 parseJSON = withObject "RoboConfig" $ \o-> do
   ...
   _color <- o .:? (pack "color")
   case _color of
     Just x -> return $ RoboConfig _rId _progName _pos (x,0)
...
initWithConfig config = do
  ...
  _<-mapM (\x-> createProcess $ shell ("./RoboInterpreterS1 " ++ show (rID x) ++ " " ++ show (progName x) ++ " " ++ show (show $ fst(color x)))) roboList
...
defCellColor = (((0,255,100),defBgColor),0)
